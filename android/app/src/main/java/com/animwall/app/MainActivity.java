...
public class MainActivity extends Activity {
    ...
    private final ArrayList<WallpaperItem> wallpaperItems = new ArrayList<>();
    private final ArrayList<ExoPlayer> livePlayers = new ArrayList<>();
    ...
    private static class WallpaperItem {
        ...
        WallpaperItem(String id, String title, String imageUrl,
                      String type, String mediaType) {
            this.id = id;
            this.title = title;
    ...
    private void createUI() {
        ...
        rootContainer.setPadding(dp(20), dp(28), dp(20), dp(40));
        ...
        wallpaperContainer.setOrientation(LinearLayout.VERTICAL);
    ...
    private void addHeader() {
        ...
        subtitle.setText("Anime wallpapers with cinematic motion");
    ...
    private void addModeButtons() {
        ...
        Button wallButton = createModeButton("🖼️ WALL", true);
        wallButton.setOnClickListener(v -> switchMode("wall"));

        Button liveButton = createModeButton("🎬 LIVE WALL", false);
        liveButton.setOnClickListener(v -> switchMode("live"));
    ...
    private Button createModeButton(String text, boolean active) {
        ...
        updateModeButton(button, active);
        return button;
    }
    ...
    private void updateModeButton(Button button, boolean active) {
        ...
        if (active) {
            bg.setColor(Color.rgb(235, 64, 166));
        } else {
            bg.setColor(Color.rgb(27, 29, 40));
            bg.setStroke(dp(1), Color.rgb(48, 51, 66));
        }
    ...
    private void switchMode(String mode) {
        currentMode = "live".equals(mode) ? "live" : "wall";

        View wall = rootContainer.findViewWithTag("wallModeButton");
        View live = rootContainer.findViewWithTag("liveModeButton");

        if (wall instanceof Button) {
            updateModeButton((Button) wall, currentMode.equals("wall"));
        }
        if (live instanceof Button) {
            updateModeButton((Button) live, currentMode.equals("live"));
        }
    ...
    private void renderCurrentMode() {
        if (wallpaperContainer == null) return;
        ...
        for (WallpaperItem item : wallpaperItems) {
            if ("live".equals(currentMode) && !item.isLive()) continue;
            if ("wall".equals(currentMode) && item.isLive()) continue;

            if (item.isLive()) addLiveCard(item);
            else addStaticCard(item);
            count++;
        }

        if (count == 0) {
            showMessage("live".equals(currentMode)
                    ? "🎬 No Live Wallpapers available"
                    : "🖼️ No Wallpapers available");
        }
    }

    private void addStaticCard(WallpaperItem item) {
        LinearLayout card = createCard();
        TextView name = createCardTitle("🖼️ " + item.title);
        ...
        Button home = createActionButton("🏠 SET HOME WALLPAPER");
        Button lock = createActionButton("🔒 SET LOCK WALLPAPER");
        Button both = createActionButton("🏠🔒 SET BOTH");
        ...
        loadBitmap(item.imageUrl, bitmap -> {
            image.setImageBitmap(bitmap);
            home.setOnClickListener(v -> setBitmapWallpaper(
                    bitmap, WallpaperManager.FLAG_SYSTEM));
            lock.setOnClickListener(v -> setBitmapWallpaper(
                    bitmap, WallpaperManager.FLAG_LOCK));
            both.setOnClickListener(v -> setBitmapWallpaper(
                    bitmap,
    ...
    // ============================================================
    // LIVE VIDEO PREVIEW - FIXED
    // ============================================================

    private void addLiveCard(WallpaperItem item) {
        LinearLayout card = createCard();
        TextView name = createCardTitle("🎬 " + item.title);
        ...
        Button live = createActionButton("✨ SET LIVE WALLPAPER");
        card.addView(live);
        wallpaperContainer.addView(card);
        ...
        try {
            ...
            player.addListener(new Player.Listener() {
                ...
                public void onPlayerError(PlaybackException error) {
                    ...
                    showLiveError(card, error);
                }
            });

            // Vercel Blob URLs are MP4 files. Explicitly declaring the
            // MIME type prevents Android from rejecting a Blob URL when
            // the server's content-type is not inferred correctly.
            ...
            android.util.Log.e("AnimeWall", "LIVE PREVIEW FAILED", e);
            playerView.setVisibility(View.GONE);
            showLiveError(card, e);
        }

        live.setOnClickListener(v -> openLiveWallpaper(item.imageUrl));
    }

    private void showLiveError(LinearLayout card, Exception error) {
        TextView errorText = new TextView(this);
        errorText.setText(
                "❌ Video cannot be played\n\n"
                        + "Android video decoder could not open this MP4."
        );
        errorText.setTextColor(Color.LTGRAY);
        errorText.setTextSize(15);
        errorText.setGravity(Gravity.CENTER);
        errorText.setPadding(dp(20), dp(30), dp(20), dp(30));
        card.addView(errorText, 1);
    }
    ...
    private LinearLayout createCard() {
        ...
        card.setPadding(0, dp(12), 0, dp(30));
        return card;
    }
    ...
    private TextView createCardTitle(String title) {
        ...
        name.setPadding(dp(4), dp(8), dp(4), dp(12));
        return name;
    }
    ...
    private void loadBitmap(String imageUrl, BitmapCallback callback) {
        new Thread(() -> {
            try {
                ...
                runOnUiThread(() -> callback.onLoaded(bitmap));
    ...
    private void setBitmapWallpaper(Bitmap bitmap, int flags) {
        try {
    ...
    private void openLiveWallpaper(String videoUrl) {
        try {
    ...
    private void showMessage(String message) {
        ...
        text.setPadding(0, dp(40), 0, dp(40));
